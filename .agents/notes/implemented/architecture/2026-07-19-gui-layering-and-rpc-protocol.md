...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) is the [desktop shell](../feature/2026-08-15-desktop-electron-shell.md): an Electron window over the same-origin HTTP carrier, running in a supervised `dsh web` child so the shell reuses the same web client packages unchanged.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Web (browser access) and the desktop shell over a supervised same-origin window; zero workspace dependencies (the registry arrives by structural injection) |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web, the vite application) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch + one assembly module per application (web.ts / headless.ts); the vite app is a thin main over the `dsh-client-web` shell surface; the desktop shell supervises the CLI's own Web boot and owns window/tray semantics only | Applications use dynamic imports so they never load each other; workspace knowledge like dist location stays in the app |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (hypothetical Electron IPC, see the "Subclass table" below). The desktop shell picks the first option in a supervised child.
2. **Write an assembly module under `apps/`**: `startHost()` + a client subclass + the application's private signal/print/exit semantics; a mixture never becomes a package — assembly is written in the app. The desktop shell instead supervises the CLI's own Web boot as a child process and adds only window/tray semantics around it.
3. **Import `dsh-host-webserver` only if you need HTTP carriage**, otherwise zero ports.

The applications preserve the division: the Web application mounts Host, carrier, and browser composition; `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports; and the desktop shell supervises the same Web application in a child process. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (hypothetical — the shipped [desktop shell](../feature/2026-08-15-desktop-electron-shell.md) reuses the Web carrier over a supervised child instead) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
