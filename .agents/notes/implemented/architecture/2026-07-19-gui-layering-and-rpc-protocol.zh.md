...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）是[桌面壳](../feature/2026-08-15-desktop-electron-shell.md)：在受监督的 `dsh web` 子进程的同源 HTTP 载体上开一个 Electron 窗口，因此壳原样复用同一套 web client 包。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | Web（浏览器访问）与受监督同源窗口下的桌面壳；零 workspace 依赖（注册表经结构注入到达） |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web，vite 应用）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发 + 每个应用一个拼装模块（web.ts / headless.ts）；vite 应用是 `dsh-client-web` 壳表面之上的薄 main；桌面壳监督 CLI 自身的 Web 启动，只拥有窗口／托盘语义 | 各应用使用动态 import，因此不会互相加载；dist 定位等 workspace 知识留在 app |
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（假想的 Electron IPC，见下文「子类表」）。桌面壳选择第一种，放在受监督的子进程里。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。桌面壳改为把 CLI 自身的 Web 启动作为子进程监督，只在其外围加窗口／托盘语义。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，否则零端口。

各应用保持这一区分：Web 应用挂载 Host、载体与浏览器组合；`dsh --profile headless` 挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口；桌面壳则在子进程中监督同一个 Web 应用。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。
...
| IPC 桥子类（假想——已发布的[桌面壳](../feature/2026-08-15-desktop-electron-shell.md)改为在受监督子进程上复用 Web 载体） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
