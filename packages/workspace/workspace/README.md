...
- `ctx.workspaceRegistry.deleteSession(id)` — prunes one already-deleted session from the registry: its id leaves every workspace record's session account (through the entity write prune: `updatedAt` stamped, change emitted), leaves the registry-global archive set, and leaves the header/path indexes. Existence was established by the caller (the stored log was deleted first); unknown ids are an idempotent no-op so re-pruning after a crash converges. The persistence deletion itself is orchestrated above the registry — see the [session deletion decision](../../../.agents/notes/implemented/feature/2026-08-15-session-deletion.md).
