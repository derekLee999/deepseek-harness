...
- `ctx.workspaceRegistry.deleteSession(id)`：从注册表中剪除一个已被删除的会话：其 id 离开每个 workspace 记录的会话账（经由实体的写剪除：盖章 `updatedAt`、发出变更）、离开注册表级全局归档集合，并离开头部/路径索引。存在性由调用方事先确认（存储日志已先删除）；未知 id 是幂等 no-op，因此崩溃后的再次剪除必然收敛。持久化删除本身在注册表之上编排——见[会话删除决策](../../../.agents/notes/implemented/feature/2026-08-15-session-deletion.md)。
