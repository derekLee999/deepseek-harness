...
- **删除。**服务 `delete(id)` 会 unlink 该会话在全部项目目录中两种物理编码的日志文件，并在可能时移除已清空的会话目录（残留目录不携带会话数据，`list` 会跳过）。它是幂等的，并且在 per-id 写链上执行，因此协调器的活动/保留/未知守卫与 `session-persistence/deleted` 事件在此处的行为与其他后端完全一致。
