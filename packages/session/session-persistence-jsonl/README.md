...
- **Deletion.** The service `delete(id)` unlinks the session's log in both physical encodings across every project directory and removes the emptied session directory when possible (a leftover directory carries no session data and `list` skips it). It is idempotent and runs on the per-id write chain, so the coordinator's live/reserved/unknown guards and the `session-persistence/deleted` event apply here exactly as on the other backends.
