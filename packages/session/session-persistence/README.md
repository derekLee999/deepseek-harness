...
| `delete(id): Promise<void>` | Permanently delete one session's stored log. Queued on the per-id write chain, so it serializes with in-flight appends and waits out a disposed live session's final drain first. An unknown id rejects; an un-materialized create intent is cancelled and resolves; a live or reserved session rejects (callers tear the session down first). After success the id behaves as unknown to every subsequent operation, and the coordinator emits `'session-persistence/deleted'(id)` strictly after the durable removal — the notified commit point derived store projections and host removal frames use (see the [session deletion decision](../../../.agents/notes/implemented/feature/2026-08-15-session-deletion.md)). |
...
| `deleteStored(id)` | Remove every durable artifact of one id (JSONL files in both physical encodings across storage scopes; SQLite `events` + `sessions` rows in one transaction). Idempotent — resolving with nothing present is valid; the coordinator calls it only after establishing the id is known, and crash-rerun convergence relies on that idempotence. |
