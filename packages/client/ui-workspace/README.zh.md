...
- **没有 Session 查看或取消归档控件**：会话可以归档与删除（删除带确认对话框并永久移除本地日志，见[会话删除决策](../../../.agents/notes/implemented/feature/2026-08-15-session-deletion.md)），但已归档会话没有查看或取消归档入口；删除 Workspace 注册记录不会删除 Session。
