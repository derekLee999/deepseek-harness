...
describe('workspace browser rows', () => {
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, archive, and delete', () => {
    ...
    // Delete IS destructive (the local log is destroyed): danger styling below the archive row.
    ...
    expect(onRename).toHaveBeenCalledOnce()
    expect(onOpen).not.toHaveBeenCalled()
    // Delete opens the confirm flow with the display title (row menu action).
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
