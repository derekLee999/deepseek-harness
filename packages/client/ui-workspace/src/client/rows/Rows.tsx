 ...
 * except workspace Rename/Delete and session Rename/Fork/Archive/Delete; the
...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onDelete, drag, flat = false, t }: {
  ...
  // confirmation dialog. Delete destroys the local log and bookkeeping: it is
  // styled as destructive and routes through a confirmation dialog.
  const sessionMenuItems = [
    { id: 'rename', label: t('rename'), icon: <IconEditOutline16 /> },
    { id: 'fork', label: t('menu.fork'), icon: <IconBranchOutline16 /> },
    // 20-native glyph in the menu's 16px icon slot (Menu.module.css .itemIcon).
    { id: 'archive', label: t('menu.archiveSession'), icon: <IconArchiveOutline20 size={16} /> },
    { id: 'deleteSession', label: t('menu.deleteSession'), icon: <IconTrashOutline16 />, danger: true },
  ]
        ...
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'deleteSession') onDelete(node.id, row.title)
            }}
            portal
            closeOnPointerLeave
            anchor={(
